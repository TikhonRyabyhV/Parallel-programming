...
void LongDouble::initFromString(const string& s, long exp) {
	size_t index;
...
LongDouble::LongDouble(long double value) {
	...
	initFromString(ss.str(), 1);
	removeZeroes();
}

LongDouble::LongDouble(const string& s, long exp) {
	initFromString(s, exp);
	removeZeroes();
}
